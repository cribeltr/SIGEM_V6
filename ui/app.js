...
(function () {
  ...
  // ===========================================================================
  // INICIO · pantalla principal del trabajo diario (interfaz simple).
  // Dos bloques: "Registrar" (eventos del día + maestro) y "Pendientes"
  // (lista priorizada con la regla de los 3 días). La tarea de inicio de mes
  // aparece solo cuando hay equipos programados sin repartir. Todo lo demás se
  // alcanza por búsqueda (⌘K) o el menú "Más". Sin lógica de negocio: usa HHHA.*
  // ===========================================================================
  VIEWS.inicio = function () {
    const S = H.getState();
    const hoy = H.hoyLocal();
    const fechaLargaCap = d => { try { const s = new Date(d + 'T00:00:00').toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' }); return s.charAt(0).toUpperCase() + s.slice(1); } catch (e) { return d; } };

    // Regla de los 3 días (igual para todos): días desde el último movimiento
    // (creación o último seguimiento). Recordarle a alguien reinicia la cuenta.
    const UMBRAL = 3;
    const ultMov = p => { let d = p.fechaCrea || hoy; (p.seguimientos || []).forEach(s => { if (s.fecha && s.fecha > d) d = s.fecha; }); return d; };
    const diasSin = p => H.diasEntreFechas(ultMov(p), hoy);

    const root = h('div', { class: 'home' });
    root.appendChild(h('div', { class: 'home-hd' }, h('h1', {}, 'Hoy'), h('div', { class: 'home-date' }, fechaLargaCap(hoy))));

    // Tarea de inicio de mes: equipos con MP programada este mes aún sin repartir
    // entre ejecutores. Desaparece sola cuando están todos asignados.
    const km = `${YEAR}-${String(MONTH + 1).padStart(2, '0')}`;
    const asign = (S.asignacionesMP || {})[km] || {};
    const sinRepartir = S.equipos.filter(e => e.estado !== 'baja' && H.mpProgramadaEnMes(e, MESES[MONTH]) && !asign[e.inv]);
    if (sinRepartir.length) {
      root.appendChild(h('button', { class: 'task-card', onclick: () => go('asignaciones', { year: YEAR, month: MONTH }) },
        h('span', { class: 'task-ic' }, svg(ic.asignaciones, 22)),
        h('span', { class: 'task-tx' },
          h('b', {}, `Inicio de mes · ${sinRepartir.length} equipo${sinRepartir.length > 1 ? 's' : ''} con mantención programada sin repartir`),
          h('span', { class: 'home-sub' }, 'Distribúyelos entre los ejecutores →')),
        svg(ic.chev, 18)));
    }

    // ===== Bloque REGISTRAR =====
    const regBtn = (label, iconPath, onclick) => h('button', { class: 'reg-btn', onclick }, h('span', { class: 'reg-ic' }, svg(iconPath, 22)), h('span', {}, label));
    root.appendChild(h('section', { class: 'card' },
      h('div', { class: 'card-hd' }, h('h2', {}, 'Registrar'), h('span', { class: 'home-sub' }, 'Lo de cada mañana')),
      h('div', { class: 'reg-grid' },
        regBtn('Mantención', ic.check, () => formNuevoEvento({ tipo: 'Mantención preventiva' })),
        regBtn('Solicitud de trabajo', ic.pendientes, () => formNuevoEvento({ tipo: 'Solicitud de trabajo' })),
        regBtn('Envío a servicio técnico', ic.ciclos, () => formNuevoEvento({ tipo: 'Envío a servicio técnico' })),
        regBtn('Recepción', ic.dl, () => formNuevoEvento({ tipo: 'Recepción' })),
        regBtn('Cargar maestro', ic.up, () => importarMaestro(() => scheduleRefresh())),
        regBtn('Otro evento…', ic.eventos, () => formNuevoEvento({})))));

    // ===== Bloque PENDIENTES (priorizado, con la regla de los 3 días) =====
    const activos = S.pendientes.filter(p => !p.anulado && p.estado !== 'cerrado');
    activos.sort((a, b) => (diasSin(b) >= UMBRAL) - (diasSin(a) >= UMBRAL) || diasSin(b) - diasSin(a));
    const nUrge = activos.filter(p => diasSin(p) >= UMBRAL).length;

    const delegar = (p, anchor) => popover(anchor, EJECUTORES.map(x => [x, () => {
      ...
      H.agregarSeguimiento(p, 'Asignado / solicitado a ' + x);
      toast('Asignado a ' + x, 'success');
    }]));
    const pendRow = p => {
      const eq = H.findEquipo(p.inv) || {};
      const dias = diasSin(p);
      const urge = dias >= UMBRAL;
      const quien = p.ejecutor;
      return h('div', { class: 'pend' + (urge ? ' urge' : '') },
        urge ? h('div', { class: 'pend-flag' }, '⏰ ' + (quien ? 'Recuérdale a ' + quien : 'Sin asignar — delégalo') + ` · ${dias} días sin avance`) : null,
        h('div', { class: 'pend-body' },
          h('div', { class: 'pend-main', onclick: () => formPendiente(p) },
            h('div', { class: 'pend-desc' }, p.desc || TIPO_PENDIENTE[p.tipo] || 'Pendiente'),
            h('div', { class: 'pend-meta' }, [eq.equipo || p.inv, quien ? 'con ' + quien : 'sin asignar', `hace ${dias} día${dias === 1 ? '' : 's'}`].join(' · '))),
          h('div', { class: 'pend-acts' },
            urge && quien ? h('button', { class: 'btn sm', title: 'Anota que ya le recordaste (reinicia la cuenta de 3 días)', onclick: () => { H.agregarSeguimiento(p, 'Recordatorio enviado a ' + quien); toast('Anotado: le recordaste a ' + quien, 'success'); } }, 'Recordar') : null,
            h('button', { class: 'btn sm', title: 'Delegar o solicitar a un ejecutor', onclick: ev => delegar(p, ev.currentTarget) }, quien ? 'Reasignar' : 'Delegar'),
            h('button', { class: 'btn sm ok', title: 'Marcar como resuelto', onclick: () => { H.cerrarPendiente(p, ''); toast('Pendiente resuelto', 'success'); } }, svg(ic.check, 14), 'Resolver'))));
    };
    root.appendChild(h('section', { class: 'card' },
      h('div', { class: 'card-hd' },
        h('h2', {}, 'Pendientes'),
        activos.length ? h('span', { class: 'count-pill' }, String(activos.length)) : null,
        nUrge ? h('span', { class: 'count-pill urge' }, nUrge + ' por recordar') : null,
        h('div', { class: 'tb-spacer' }),
        h('button', { class: 'btn sm', onclick: () => formNuevoPendiente({}) }, svg(ic.plus, 14), 'Nuevo')),
      activos.length ? h('div', { class: 'pend-list' }, ...activos.map(pendRow)) : h('div', { class: 'empty-calm' }, '✓ Sin pendientes. Todo al día.')));
  ...
  function boot() {
    ...
    // Chrome simple y calmado: marca · buscador prominente (⌘K) · tema · menú "Más".
    // La navegación densa anterior (rail/topnav) se retiró; todo se alcanza por
    // búsqueda o por el menú "Más". El inicio es la pantalla de trabajo diario.
    const app = h('div', { class: 'app' },
      h('header', { class: 'topbar' },
        h('div', { class: 'brand', title: 'SIGEM · Gestión de equipos críticos', style: { cursor: 'pointer' }, onclick: () => go('inicio') }, h('span', { class: 'mark' }, 'S'), h('span', { class: 'brand-name' }, 'SIGEM')),
        h('div', { class: 'tb-spacer' }),
        h('button', { class: 'search-pill', title: 'Buscar un equipo y abrir su ficha', onclick: () => openCmdk() }, svg(ic.search, 16), h('span', { class: 'muted' }, 'Buscar equipo…'), h('span', { class: 'kbd s-hide' }, '⌘K')),
        h('button', { class: 'btn icon ghost', id: 'btn-theme', title: 'Tema claro / oscuro', onclick: () => applyTheme(document.documentElement.getAttribute('data-theme') === 'dark' ? 'light' : 'dark') }),
        h('button', { class: 'btn icon ghost', title: 'Más', onclick: ev => menuMas(ev.currentTarget) }, svg(ic.dots, 18)),
